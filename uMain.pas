...
type
  ...
  TfrmMain = class(TForm)
    ...
    tbsVisualizarPendencias: TTabSheet;
    lblListaArquivosProcessados: TLabel;
    chklstListaArquivosProcessados: TCheckListBox;
    lblListaDeArquivosPendentes: TLabel;
    lblTotalDeArquivosProcessados: TLabel;
    lblTotalDeArquivospendentes: TLabel;
    lstListaDeArquivosPendentes: TListBox;
    chkTeste: TCheckBox;
    btnExecutar: TBitBtn;
    btnSelecionarTodosTrack: TBitBtn;
    btnLimparSelecaoTrack: TBitBtn;
    btnAtualizar: TButton;
    tmrAtualizaDados: TTimer;
    btnExportarPendencias: TButton;
    ...
    procedure btnSelecionarTodosTrackClick(Sender: TObject);
    procedure btnLimparSelecaoTrackClick(Sender: TObject);
    procedure btnAtualizarClick(Sender: TObject);
    procedure tmrAtualizaDadosTimer(Sender: TObject);

    procedure ExportarPendencias();
    procedure btnExportarPendenciasClick(Sender: TObject);
  ...
  private
    ...
    procedure AtualizarQtdeArquivosMarcadosTrack();
    procedure AtualizarQtdeArquivosMarcadosReverter();
    procedure LimparSelecaoTrack();
    procedure SelecionarTodosTrack();
   ...
   {Converte String em Pchar}
    ...
    procedure FRM_VERIFICA_ARQUIVOS();

    procedure FRM_HABILITA_BOTAO(BOTAO: TBitBtn);
    PROCEDURE FRM_DESAHABILITA_BOTAO(BOTAO: TBitBtn);
...
begin
  ...
  try
    try
      ...
      for iContArquivosSelecionados:= 0 to chklstListaArquivosProcessados.Count - 1 do
        if chklstListaArquivosProcessados.Checked[iContArquivosSelecionados] then
          ListaDeArquivosSelecionados.Add(Trim(objCore.objString.getTermo(2, ' - ', chklstListaArquivosProcessados.Items[iContArquivosSelecionados])));
    ...
    case MessageBox (Application.Handle, Pchar ('FIM DE PROCESSAMENTO !'+#13#10#13#10+'Deseja abrir a pasta de saída? '),
     'Abrir pasta de saída', MB_YESNO) of
      idYes:  objCore.objFuncoesWin.ExecutarArquivoComProgramaDefault(objCore.objParametrosDeEntrada.PATHSAIDA);
    end;
...
begin
  ...
  TRY
    ...
    CloseFile(flUser);
          ...
          tbsExecutar.TabVisible    := False;

          lblTotalDeArquivosProcessados.Caption := '';
          lblTotalDeArquivospendentes.Caption := '';

          tbsVisualizarPendencias.PageIndex := 0;
          tbsReverter.PageIndex := 1;
  ...
  try
    try
      ...
      begin
        ...
        if (not objCore.ArquivoExieteTabelaTrackLine(sltListaDeArquivos.Strings[iContArquivo])) or (chkTeste.Checked) then
...
procedure TfrmMain.LimparSelecaoTrack();
var
  i: integer;
begin
  {Itera pela CheckListBox e marca cada item (checked = true)}

  for i:=0 to chklstListaArquivosProcessados.Items.Count-1 do
  begin
    if chklstListaArquivosProcessados.Checked[i] then
      chklstListaArquivosProcessados.Checked[i] := false;
  end;

  AtualizarQtdeArquivosMarcados();
end;

procedure TfrmMain.SelecionarTodosTrack();
var
  i: integer;
begin
 {Itera pela CheckListBox e marca cada item (checked = true)}
  ...
  for i:=0 to chklstListaArquivosProcessados.Items.Count-1 do
    chklstListaArquivosProcessados.Checked[i] := true;
...
begin
  ...
  lblInfos.Caption := inttostr(iTotalMarcados) + ' arquivo(s) marcado(s) - '  + objCore.objFuncoesWin.GetTamanhoMaiorUnidade(iTamArquivos);
...
procedure TfrmMain.AtualizarQtdeArquivosMarcadosTrack();
var
  j                     : integer;
  iTotalMarcados        : integer;
begin

  //===============================================================
  //  LISTA DE ARQUIVOS PROCESSADOS PRONTOS PARA GERAR TRACK
  //===============================================================
  iTotalMarcados := 0;
  for j:=0 to chklstListaArquivosProcessados.Items.Count-1 do
  begin

    if chklstListaArquivosProcessados.Checked[j] then
    begin
  ...
  //==============================
  //  DESABILITA BOTAO EXECUTAR
  //==============================
  IF iTotalMarcados > 0 THEN
    FRM_HABILITA_BOTAO(btnExecutar)
  ELSE
    FRM_DESAHABILITA_BOTAO(btnExecutar);

  lblTotalDeArquivosProcessados.Caption := IntToStr(chklstListaArquivosProcessados.Items.Count) +  ' arquivo(s) processado(s) | ' + inttostr(iTotalMarcados) + ' arquivo(s) marcado(s)';
  lblTotalDeArquivosProcessados.Refresh;
  Application.ProcessMessages;
  //===============================================================
  ...
  //===============================================================
  lblTotalDeArquivospendentes.Caption := IntToStr(lstListaDeArquivosPendentes.Items.Count) +  ' arquivo(s) pendentes(s)';
  lblTotalDeArquivospendentes.Refresh;
  Application.ProcessMessages;
  //===============================================================
...
procedure TfrmMain.btnSelecionarTodosTrackClick(Sender: TObject);
begin
  SelecionarTodosTrack();
end;

procedure TfrmMain.btnLimparSelecaoTrackClick(Sender: TObject);
begin
  LimparSelecaoTrack();
end;

procedure TfrmMain.FRM_VERIFICA_ARQUIVOS();
begin

  chklstArquivosProcessados.Clear;
  lstListaDeArquivosPendentes.Clear;

  objCore.getListaDeArquivosPendentes();
  objCore.getListaDeArquivosTrack();

  chklstListaArquivosProcessados.Items := objCore.objParametrosDeEntrada.STL_LISTA_ARQUIVOS_TRACK;
  lstListaDeArquivosPendentes.Items    := objCore.objParametrosDeEntrada.STL_LISTA_ARQUIVOS_PENDENTES;

  AtualizarQtdeArquivosMarcadosTrack();
...
procedure TfrmMain.tmrAtualizaDadosTimer(Sender: TObject);
begin
  AtualizarQtdeArquivosMarcadosTrack();
  AtualizarQtdeArquivosMarcadosReverter();
end;

procedure TfrmMain.FRM_HABILITA_BOTAO(BOTAO: TBitBtn);
begin
  BOTAO.Enabled := True;
end;

procedure TfrmMain.FRM_DESAHABILITA_BOTAO(BOTAO: TBitBtn);
begin
  BOTAO.Enabled := False;
end;


procedure TfrmMain.ExportarPendencias();
begin

  lstListaDeArquivosPendentes.Items.SaveToFile(ExtractFilePath(Application.ExeName) + 'LISTA_PENDENCIAS.TXT');
  objCore.objFuncoesWin.ExecutarArquivoComProgramaDefault(ExtractFilePath(Application.ExeName) + 'LISTA_PENDENCIAS.TXT');

end;

procedure TfrmMain.btnExportarPendenciasClick(Sender: TObject);
begin
  ExportarPendencias();
end;
